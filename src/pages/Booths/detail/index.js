...
import { Row, Col, Typography, Input, Button, Form, Select, Tag, Space, Spin, Image, Divider, Result } from 'antd';
...
const BoothDetail = () => {
    ...
    if (loadingBooth) return <div className="wapperLoading"><Spin tip="Đang tải dữ liệu..." /></div>
    return (
        <Fragment>
            {
                (!booth && !loadingBooth) ? (
                    ...
                    <div className={clsx(style.customerPage)}>
                        <Row style={{ marginBottom: 24 }} justify='space-between'>
                            <div className={clsx(style.boxTitleCreate)}>
                                <ArrowBack style={{ cursor: 'pointer', marginRight: 12 }} onClick={() => history.push('/booths')} />
                                <Title level={3}>Chi tiết kho hàng lưu động</Title>
                            </div>
                            <Button icon={<PlusOutlined />} onClick={() => history.push(`/booths/edit/${id}`)} className={clsx(style.addBtn)}>Sửa thông tin</Button>
